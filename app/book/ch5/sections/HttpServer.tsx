...
# v22.11.0`}</CodeBlock>
      ...
      >
        <p>
          ...
          <code>web-dev-client</code>. The two directories are{" "}
          <strong>siblings</strong>{" "}under the same parent:
        </p>
        <CodeBlock language="shell">{`mkdir web-dev-server
cd web-dev-server
npm init`}</CodeBlock>
        <p>
          ...
          repo root as <code>web-dev-server/</code>{" "}so LiveDemos
      ...
      >
        ...
        <CodeBlock language="js" name="Hello" file="web-dev-server/Hello.js">{`console.log("Hello World!");`}</CodeBlock>
...
# later: npm run dev   (same command, after the script in 5.1.7)`}</CodeBlock>
        ...
        <WithAI
          prompt={`In web-dev-server/index.js, keep my /hello route. Add a sample GET / that sends Welcome to Full Stack Development! Do not remove hello.`}
        >
      ...
      >
        ...
        <CodeBlock language="json" name="package" file="web-dev-server/package.json">{`{
  "type": "module",
  "scripts": {
    "start": "node index.js",
